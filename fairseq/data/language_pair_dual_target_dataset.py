...
def collate(
    ...
    if samples[0].get("target1", None) is not None:
        ...
        ntokens1 = tgt1_lengths.sum().item()
        ...
        ntokens2 = tgt2_lengths.sum().item()


        if samples[0].get("prev_output_tokens1", None) is not None:
            prev_output_tokens1 = merge("prev_output_tokens1", left_pad=left_pad_target)
        ...
        ntokens1 = ntokens2 = src_lengths.sum().item()
    ...
    if prev_output_tokens1 is not None:
        batch["net_input"]["prev_output_tokens1"] = prev_output_tokens1.index_select(
            0, sort_order
        )
        batch["net_input"]["prev_output_tokens2"] = prev_output_tokens2.index_select(
            0, sort_order
        )
...
class LanguagePairDualTargetDataset(FairseqDataset):
    """
    A triplet of torch.utils.data.Datasets. Source -> Target1 / Target2.
    Assume target 1 and 2 have the same dict.

    Args:
        src (torch.utils.data.Dataset): source dataset to wrap
        src_sizes (List[int]): source sentence lengths
        src_dict (~fairseq.data.Dictionary): source vocabulary
        tgt1 (torch.utils.data.Dataset, optional): target dataset to wrap
        tgt1_sizes (List[int], optional): target sentence lengths
        tgt1_dict (~fairseq.data.Dictionary, optional): target vocabulary
        tgt2 (torch.utils.data.Dataset, optional): target dataset to wrap
        tgt2_sizes (List[int], optional): target sentence lengths
        tgt2_dict (~fairseq.data.Dictionary, optional): target vocabulary
    ...
    def __init__(
        ...
        if tgt1 is not None:
            assert len(src) == len(tgt1) == len(tgt2), "Source and target must contain the same number of examples"
        self.src = src
        self.tgt1 = tgt1
        self.tgt2 = tgt2
        ...
        self.sizes = (
            np.vstack((self.src_sizes, self.tgt1_sizes)).T
        ...
        if num_buckets > 0:
            ...
            if self.tgt1 is not None:
                ...
                logger.info(
                    "bucketing target lengths: {}".format(list(self.tgt1.buckets))
                )
            if self.tgt2 is not None:
                ...
                logger.info(
                    "bucketing target lengths: {}".format(list(self.tgt2.buckets))
                )
    ...
    def ordered_indices(self):
        ...
        if self.buckets is None:
            # sort by target2 length, target1 length,then source length
    ...
    def filter_indices_by_size(self, indices, max_sizes):
        ...
        """
        return data_utils.filter_paired_dual_target_dataset_indices_by_size(
            self.src_sizes,
            self.tgt1_sizes,
            self.tgt2_sizes,
            indices,
